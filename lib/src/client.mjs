...
class ConnectMeClient {
	...
	/** @return {Promise} */
	#processHandler = async (/** function */ handler, /** string|Uint8Array|PackMeMessage */ data) => {
		try {
			await handler(data);
		}
		catch (err) {
	...
	/** @return {Promise} */
	async query(/** PackMeMessage */ message) {
		let completer = {};
		...
		if (data != null && this.socket != null && this.socket.readyState === WebSocket.OPEN) {
			this.#queries.set(message.$transactionId, Query(completer));
